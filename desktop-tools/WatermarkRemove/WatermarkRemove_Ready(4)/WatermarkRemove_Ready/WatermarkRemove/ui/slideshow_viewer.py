...
class SlideshowViewer(QDialog):
    ...
    def __init__(self, folder_path: str, parent=None, watermark_folder: str = None, watermark_name: str = None, watermark_tab=None):
        ...
        self.auto_timer.timeout.connect(self._auto_process_step)
    ...
    def _create_controls_panel(self) -> QWidget:
        ...
        # --- Auto-Proceso ---
        auto_group = QGroupBox("⚡ Auto-Proceso")
        auto_layout = QVBoxLayout(auto_group)
        auto_layout.setSpacing(4)

        self.auto_detect_checkbox = QCheckBox("Detectar variantes automáticamente")
        self.auto_detect_checkbox.setChecked(True)
        auto_layout.addWidget(self.auto_detect_checkbox)

        self.auto_status_label = QLabel("Listo.")
        self.auto_status_label.setWordWrap(True)
        self.auto_status_label.setStyleSheet("color: #aaa; font-size: 10px;")
        auto_layout.addWidget(self.auto_status_label)

        auto_btn_row = QHBoxLayout()
        self.auto_start_btn = QPushButton("▶ Iniciar Auto")
        self.auto_start_btn.setStyleSheet("background-color: #388E3C; color: white; padding: 6px; font-weight: bold;")
        self.auto_start_btn.clicked.connect(self._start_auto_process)
        self.auto_stop_btn = QPushButton("⏹ Detener")
        self.auto_stop_btn.setStyleSheet("background-color: #c62828; color: white; padding: 6px;")
        self.auto_stop_btn.setEnabled(False)
        self.auto_stop_btn.clicked.connect(self._stop_auto_process)
    ...
    def _auto_detect_best_candidate(self, image_path: Path):
        """
        Detecta la mejor marca de agua usando template matching libre en toda la imagen.
        Devuelve (pos_name, wm_index, score) donde pos_name es una clave sintética,
        o None si no se detecta con suficiente confianza.
        """
        try:
        ...
        for wm_idx in wm_indices:
            ...
            if wm_h >= h_img or wm_w >= w_img:
                continue

            # Crear template: marca compuesta sobre fondo blanco
            wm_color = watermark[:, :, :3]
            alpha = watermark[:, :, 3:4].astype(np.float32) / 255.0
            bg = np.ones_like(wm_color, dtype=np.float32) * 255
            template = (wm_color.astype(np.float32) * alpha + bg * (1 - alpha)).astype(np.uint8)
            template_gray = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)

            # Template matching en toda la imagen
            result = cv2.matchTemplate(img_gray, template_gray, cv2.TM_CCOEFF_NORMED)
            _, max_val, _, max_loc = cv2.minMaxLoc(result)
            x, y = max_loc

            # Convertir score: más alto en matchTemplate = mejor → invertir para "score menor = mejor"
            match_score = 1.0 - max_val
        ...
        if best_wm_idx is None or best_x is None:
            return None

        # Umbral mínimo de confianza: score < 0.55 (= correlación > 0.45)
        CONFIDENCE_THRESHOLD = 0.55
        if best_score > CONFIDENCE_THRESHOLD:
            return None

        # Construir un pos_data sintético con coordenadas absolutas
        # Usamos side_x/side_y=top/left con offset = coordenada absoluta
        # Esto permite reutilizar remove_watermark directamente
        wm = load_images_cv2(self.watermark_files[best_wm_idx])
        wm_h, wm_w = wm.shape[:2]
        h_img, w_img = image.shape[:2]

        # Determinar el anchor más limpio (menor offset absoluto)
        cx = best_x + wm_w // 2
        cy = best_y + wm_h // 2

        if cx / w_img < 0.33:
            side_x = "left"; off_x = best_x
        elif cx / w_img > 0.67:
            side_x = "right"; off_x = w_img - wm_w - best_x
        else:
            side_x = "center"; off_x = best_x - (w_img - wm_w) // 2

        if cy / h_img < 0.33:
            side_y = "top"; off_y = best_y
        elif cy / h_img > 0.67:
            side_y = "bottom"; off_y = h_img - wm_h - best_y
        else:
            side_y = "center"; off_y = best_y - (h_img - wm_h) // 2

        pos_name = f"auto_{side_x}_{side_y}"
        pos_data = {"offset_x": off_x, "offset_y": off_y, "side_x": side_x, "side_y": side_y}

        # Registrar la posición detectada para poder dibujar el overlay
        self.watermark_positions[pos_name] = pos_data

        return (pos_name, best_wm_idx, best_score)

    def _score_candidate(self, image: np.ndarray, watermark: np.ndarray, rd: dict) -> float:
        """Score menor = más probable que la marca esté ahí. (Mantenido para compatibilidad)"""
        try:
            x, y = align_watermark(
                image, watermark,
        ...
        x_start_img = max(0, x); y_start_img = max(0, y)
        x_end_img = min(w_img, x + w_wm); y_end_img = min(h_img, y + h_wm)
        if x_start_img >= x_end_img or y_start_img >= y_end_img:
            return float("inf")

        x_start_wm = max(0, -x); y_start_wm = max(0, -y)
        ...
        diff_score = float(diff[mask].mean())
        alpha3 = alpha[:, :, None]
        alpha_safe = np.clip(1.0 - alpha3, 1e-5, 1.0)
        bg_raw = (roi / alpha_safe) - (wm_crop * (alpha3 / alpha_safe))
        clip_err = float(np.abs(bg_raw - np.clip(bg_raw, 0, 255))[mask].mean())
        out_ratio = float(((bg_raw < 0.0) | (bg_raw > 255.0))[mask].mean())
