...
class SlideshowViewer(QDialog):
    ...
    def _auto_detect_best_candidate(self, image_path: Path):
        ...
        # Heurística de confianza dinámica:
        # - Si el score es muy bueno (< 60), siempre aceptar
        # - Si hay poca separación con el segundo candidato Y el score es alto, rechazar
        n_candidates = len(pos_candidates) * len(wm_indices)
        margin_required = 4.0 if n_candidates <= 3 else 6.0
        score_threshold = 130.0 if n_candidates <= 3 else 110.0

        if best[2] > score_threshold and (second - best[2]) < margin_required:
            return None
